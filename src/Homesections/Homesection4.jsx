...
const trends = [
  ...
  },
  {
    ...
    icon: <BarChart3 className="w-6 h-6 sm:w-8 sm:h-8" />, // Fixed: was Globe, now BarChart3 for relevance
    linear: 'from-orange-600 to-red-600', // Added unique gradient to avoid visual repetition
  }
];

export default function Homesection4() {
  return (
    <section className="relative py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 overflow-hidden bg-gradient-to-br from-[#0f0020] via-[#1a0033] to-[#0f0f1e]">
      {/* Background Glows */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-80 h-80 sm:w-96 sm:h-96 bg-purple-800/40 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 sm:w-96 sm:h-96 bg-pink-800/30 rounded-full blur-3xl translate-x-1/2 translate-y-1/2" />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto text-center">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl font-black text-white mb-3 sm:mb-4">
          MEGA TRENDS
        </h2>
        <p className="text-gray-300 text-sm sm:text-base md:text-lg max-w-4xl mx-auto mb-4 sm:mb-6 px-2 sm:px-4">
          by DigitalAdda Agency
        </p>
        <p className="text-gray-400 text-xs sm:text-sm md:text-base max-w-4xl mx-auto mb-6 sm:mb-8 px-2 sm:px-4 leading-relaxed">
        ...
        <div className="w-20 h-0.5 sm:w-24 md:w-32 bg-gradient-to-r from-purple-500 to-pink-500 mx-auto rounded-full shadow-lg shadow-purple-500/80" />

        {/* Trends Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 mt-10 sm:mt-14 md:mt-16 lg:mt-20">
          {trends.map((trend) => (
            <div
              key={trend.number}
              className="group relative p-4 sm:p-5 md:p-6 lg:p-7 xl:p-8 rounded-xl sm:rounded-2xl lg:rounded-3xl bg-white/5 backdrop-blur-xl border border-purple-500/30
                         hover:border-purple-400 hover:bg-white/10 transition-all duration-500 cursor-pointer
                         shadow-lg hover:shadow-xl hover:shadow-purple-500/20"
            >
              {/* Linear Hover Overlay */}
              <div
                className={`absolute inset-0 rounded-xl sm:rounded-2xl lg:rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700
                            bg-gradient-to-br ${trend.linear} blur-xl -z-10`}
              />

              {/* Number + Icon Badge */}
              <div className="flex items-center justify-between mb-3 sm:mb-4 md:mb-5">
                <span className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">
                  {trend.number}
                </span>
                <div
                  className={`w-10 h-10 sm:w-11 md:w-12 lg:w-14 rounded-lg sm:rounded-xl md:rounded-2xl bg-gradient-to-br ${trend.linear} p-2.5 sm:p-3 flex items-center justify-center text-white shadow-md 
              ...
              {/* Title */}
              <h3 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-white mb-2 sm:mb-3 leading-tight">
                {trend.title}
              </h3>

              {/* Description */}
              <p className="text-gray-300 text-xs sm:text-sm leading-relaxed mb-4 sm:mb-5 line-clamp-4">
                {trend.desc}
              </p>

              {/* Arrow */}
              <div className="flex justify-end">
                <ArrowRight className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 text-purple-400 group-hover:text-pink-400 group-hover:translate-x-1.5 transition-all duration-300" />
              </div>
            </div>
          ))}
        </div>
